...
lcminigames.minigames.set("generation", {
	...
	getRewards() {
		return [
			...
			{
				item: Item.of("deeperdarker:warden_upgrade_smithing_template"),
				chance: 50
			},
			// Ingots and Ores
			...
			{
				item: Item.of("createfood:grilled_cheese_sandwich", 5),
				chance: 100
			},
			...
			{
				item: Item.of("pamhc2foodcore:berrysmoothieitem", 5),
				chance: 150
			},
			{ item: Item.of("pamhc2foodcore:milkshakeitem", 5), chance: 150 },
			{
				item: Item.of("pamhc2foodcore:smoothieberryitem", 5),
				chance: 150
			},
	...
	execute({ server }: $ServerKubeEvent_) {
		...
		server.tell(
			global.lcminigames.createChatFrame("Server Minigame", [
				...
				global.lcminigames.centeredMessage(
					`§bGeneration ${this.generation}`
				)
			])
		);
